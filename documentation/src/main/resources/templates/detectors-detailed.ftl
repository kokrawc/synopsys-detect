...
entry point, and detector.  This page contains a similar table, but adds additional details (entry point yielding and nesting rules) not present on the Detectors page.
...
|Detector Type|Entry Point|Detector|Language|Forge|Entry Point Search Rules / Detector Requirements|Accuracy|
|---|---|---|---|---|---|---|
<#list detectorTypes as detectorType>
|**${detectorType.name}**|||||||
<#list detectorType.entryPoints as entryPoint>
||${entryPoint.name}|||| **Yields to**: <#list entryPoint.searchRule.yieldsTo as yieldsTo>${yieldsTo} </#list> ||
